...
{	// Action constructor
	...
	switch(this.sName)
	{
		...
		case "GOSUB":
		{
			CL.sReturnFrameId = CLZ.sCurrentFrameId; // break omitted intentionally
		}
		case "GOTO":
		{
			if(oArgs.bDynamic)
					...
					if(this.oParams.bInstant)
					{
						for(var i=0; i<this.aToReset.length; i++)
						{
							...
							if(this.aToReset[i].bPosition)
							{
								...
								if(navigator.platform=="iPhone" || (navigator.platform==null && navigator.userAgent.indexOf("iPhone")!=-1))
								{
									setTimeout(function () { $(oCurObj.div).css({ "left": oCurObj.initial.x + "px", "top": oCurObj.initial.y + "px" }); }, 400);
								}
								else
								{
					...
					for(var i=0; i<this.aTargets.length; i++)
					{
						...
						switch(this.oParams.sType)
						{
							case "move": // Move done
							{
								...
								if(oCurObj.oStates!=null)
								{
									oCurObj.oStates["default"].current.x = this.aTargets[i].nEndX;
									oCurObj.oStates["default"].current.y = this.aTargets[i].nEndY;
									if(oCurObj.oStates["highlighted"]!=null)
									{
										oCurObj.oStates["highlighted"].current.x = this.aTargets[i].nEndX;
										oCurObj.oStates["highlighted"].current.y = this.aTargets[i].nEndY;
									}
								}
...
{	// CLObject methods
		...
		if(oArgs!=null && oArgs.bUseTransition!=null)
		{
			if(oArgs.bActionCall==true)
			{
				...
				else
				{
					...
					var oOptions =
					{
						effect: CL.aTransitionsCommon[iIdx].t,
						duration: oArgs.iDuration,
						complete: function ()
						{
							if(CLZ.nZoom!=1 && $(oThis.div).attr("cl-fix")=="1")
							{
								$(oThis.div).removeAttr("cl-fix").css({ "left": oThis.current.x + "px", "top": oThis.current.y + "px" });
							}
							CL.Event.Fire({ aEvents: [{ sEvtName: "ondisplaytransitionend", sSourceType: "OBJECT", sSourceId: oThis.sId } ]});
						}
					};
					...
					if(CLZ.nZoom!=1 && iIdx!=12 && iIdx!=20)
					{
						$(oThis.div).attr({ "cl-fix": "1" }).css({ "left": oThis.current.x/CLZ.nZoom + "px", "top": oThis.current.y/CLZ.nZoom + "px" });
					}
	...
	CLObject.prototype.Render = function (oArgs)
	{
		...
		$(oThis.div).hide().css({ "visibility": "visible" });
		if(oThis.initial.bDraggable)
		{
			$(oThis.div).draggable(
			{
				...
				drag: function (event, ui)
				{
					if(CLZ.nZoom!=1)
					{
						var nWidth = ui.helper.parent().width(), nHeight = ui.helper.parent().height();
						ui.position.left = Math.max(0, Math.min(ui.position.left / CLZ.nZoom , nWidth - ui.helper.width()));
						ui.position.top = Math.max(0, Math.min(ui.position.top  / CLZ.nZoom,  nHeight - ui.helper.height()));
					}
					var sId = this.getAttribute("id");
					if(ui!=null)
					{
				...
				stop: function (event, ui)
				{
					...
					if(ui!=null)
					{
						var oPosition = $(this).position();//ui.position;
						CLO[sId].current.x = oPosition.left/CLZ.nZoom;
						CLO[sId].current.y = oPosition.top/CLZ.nZoom;
						if(CL.bTouch && CL.bIOS)
						{
		...
		if(oThis.jxNode.attr("ondrop")!=null)
		{
			$(oThis.div).droppable(
			{
				...
				{
					if(CLZ.nZoom!=1 && oThis.sType=="101_clickarea_active")
					{
						$(this).width($(this).width()*CLZ.nZoom).height($(this).height()*CLZ.nZoom);
					}
				},
				drop: function (event, ui)
				{
...
var CL =
{
		...
		Key: function (oArgs)
		{
			...
			switch(oArgs.oEvt.type)
			{
				case "keydown":
				{
					...
					for(sKey in CLF)
					{
						if(CLF[sKey].bMaster==true)
						{
							continue; // or allow master key events?
						}
						if(CLF[sKey].bActive==true)
						{
							if(CLF[sKey].jxKeyEvents!=null)
							{
								for(i=0; i<CLF[sKey].jxKeyEvents.length; i++)
								{
									if(CL.Event.KeyCheck({ oEvt: oArgs.oEvt, xNode: CLF[sKey].jxKeyEvents[i] }))
									{
										aToRun.push( $(CLF[sKey].jxKeyEvents[i]) );
									}
								}
									}
								}
					...
					if(aToRun.length>0)
					{
						for(i=0; i<aToRun.length; i++)
						{
							CL.Resp.Process({ jxResp: aToRun[i] });
						}
					}
					aToRun = [];
					for(sKey in CLS)
					{
						if(CLS[sKey].bActive==true)
						{
							...
							{
								for(i=0; i<CLS[sKey].jxKeyEvents.length; i++)
								{
									if(CL.Event.KeyCheck({ oEvt: oArgs.oEvt, xNode: CLS[sKey].jxKeyEvents[i] }))
									{
										aToRun.push( $(CLS[sKey].jxKeyEvents[i]) );
									}
								}
							}
						}
	...
	Resp:
	{
		...
		Process: function (oArgs)
		{
			...
			{
				if(oArgs.evt instanceof TouchEvent)
				{
					oArgs.evt.button = 0;
				}
